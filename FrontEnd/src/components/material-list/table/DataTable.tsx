...
export const initialDataMaterial: DataMaterial[] = [
  {
    id: "1",
    name: "Dương Hân",
    gender: "Female",
...
interface DataTableProps {
  onSelectMaterial: (material: DataMaterial) => void;
}

const DataTable: React.FC<DataTableProps> = ({ onSelectMaterial }) => {
  const [data, setData] = useState<DataMaterial[]>(initialDataMaterial);
 ...
 // Handle selection for a single row
  const handleRowSelect = (id: string) => {
    ...
    const selectedMaterial = data.find((material) => material.id === newSelectedId) || null;
    if (selectedMaterial) {
      onSelectMaterial(selectedMaterial); // Gửi khách hàng đã chọn lên ActionButton
    }
  };
  ...
  const totalPages = Math.ceil(initialDataMaterial.length / quantity);
  const startIndex = (currentPage - 1) * quantity;
  const endIndex = startIndex + quantity;
  const currentData = initialDataMaterial.slice(startIndex, endIndex);
  const items = {
    currentPage,
    setCurrentPage,
    totalPages,
