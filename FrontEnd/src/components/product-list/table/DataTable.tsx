...
const DataTable: React.FC<DataTableProps> = ({ onSelectProduct }) => {
 ...
 // Handle selection for a single row
  const handleRowSelect = (id: string) => {
    ...
    const selectedProduct = data.find((product) => product.id === newSelectedId) || null;
    if (selectedProduct) {
      onSelectProduct(selectedProduct); // Gửi khách hàng đã chọn lên ActionButton
    }
  };
  ...
  const currentData = initialDataProduct.slice(startIndex, endIndex);
  const items = {
    currentPage,
    setCurrentPage,
    totalPages,
